...
对于你想搬移的每一个字段，运用[[../第 8 章 搬移特性/8.2 Move Field|搬移字段]]（207）搬移之。每次更改后运行测试。

使用[[../第 8 章 搬移特性/8.1 Move Function|搬移函数]]（198）将必要函数搬移到新类。先搬移较低层函数（也就是“被其他函数调用”多于“调用其他函数”者）。每次更改后运行测试。

检查两个类的接口，去掉不再需要的函数，必要时为函数重新取一个适合新环境的名字。

决定是否公开新的类。如果确实需要，考虑对新类应用[[../第 9 章 重新组织数据/9.4 Change Reference to Value|将引用对象改为值对象]]（252）使其成为一个值对象。
...
现在，我运用[[../第 8 章 搬移特性/8.2 Move Field|搬移字段]]（207）搬移一个字段。
...
“电话号码”对象一般还具有复用价值，因此我考虑将新提炼的类暴露给更多的客户端。需要访问 TelephoneNumber 对象时，只须把 Person 类中那些 office 开头的访问函数搬移过来并略作修改即可。但这样 TelephoneNumber 就更像一个值对象（Value Object）[mf-vo]了，因此我会先对它使用[[../第 9 章 重新组织数据/9.4 Change Reference to Value|将引用对象改为值对象]]（252）（那个重构手法所用的范例，正是基于本章电话号码例子的延续）。
