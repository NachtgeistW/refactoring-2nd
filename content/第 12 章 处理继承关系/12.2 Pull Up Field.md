---
title: 12.2 字段上移（Pull Up Field）
aliases:
  - 字段上移
---

反向重构：[[第 12 章 处理继承关系/12.5 Push Down Field|字段下移]]（361）
...
许多动态语言不需要在类定义中定义字段，相反，字段是在第一次被赋值的同时完成声明。在这种情况下，字段上移基本上是应用[[第 12 章 处理继承关系/12.3 Pull Up Constructor Body|构造函数本体上移]]（355）后的必然结果。
