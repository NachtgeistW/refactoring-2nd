...
我的下一个重构目标是 volumeCredits。处理这个变量更加微妙，因为它是在循环的迭代过程中累加得到的。第一步，就是应用[[第 8 章 搬移特性/8.7 Split Loop|拆分循环]]（227）将 volumeCredits 的累加过程分离出来。
...
完成这一步，我就可以使用[[第 8 章 搬移特性/8.6 Slide Statements|移动语句]]（223）手法将变量声明挪动到紧邻循环的位置。
...
- 使用[[第 8 章 搬移特性/8.7 Split Loop|拆分循环]]（227）分离出累加过程；
- 使用[[第 8 章 搬移特性/8.6 Slide Statements|移动语句]]（223）将累加变量的声明与累加过程集中到一起；
...
现在我们已经有了安放 play 字段的地方，可以把数据放进去。我需要对 playFor 和 statement 函数应用[[第 8 章 搬移特性/8.1 Move Function|搬移函数]]（198）（然后编译、测试、提交）。
...
等到搬移完成，编译、测试、提交也做完，我便忍不住[[第 8 章 搬移特性/8.8 Replace Loop with Pipeline|以管道取代循环]]（231）对几个地方进行重构。
...
我的设想是先建立一个继承体系，它有“喜剧”（comedy）和“悲剧”（tragedy）两个子类，子类各自包含独立的计算逻辑。调用者通过调用一个多态的 amount 函数，让语言帮你分发到不同的子类的计算过程中。volumeCredits 函数的处理也是如法炮制。为此我需要用到多种重构方法，其中最核心的一招是[[第 10 章 简化条件逻辑/10.4 Replace Conditional with Polymorphism|以多态取代条件表达式]]（272），将多个同样的类型码分支用多态取代。但在施展[[第 10 章 简化条件逻辑/10.4 Replace Conditional with Polymorphism|以多态取代条件表达式]]（272）之前，我得先创建一个基本的继承结构。我需要先创建一个类，并将价格计算函数和观众量积分计算函数放进去。
...
我要搬移的下一块逻辑，对计算一场演出的价格（amount）来说就尤为重要了。在调整嵌套函数的层级时，我经常将函数挪来挪去，但接下来需要改动到更深入的函数上下文，因此我将小心使用[[第 8 章 搬移特性/8.1 Move Function|搬移函数]]（198）来重构它。首先，将 amount 函数的逻辑复制一份到新的上下文中，也就是 PerformanceCalculator 类中。然后微调一下代码，将 aPerformance 改为 this.performance，将 playFor(aPerformance)改为 this.play，使代码适应这个新家。
...
我已将全部计算逻辑搬移到一个类中，是时候将它多态化了。第一步是应用[[第 12 章 处理继承关系/12.6 Replace Type Code with Subclasses|以子类取代类型码]]（362）引入子类，弃用类型代码。为此，我需要为演出计算器创建子类，并在 createStatementData 中获取对应的子类。要得到正确的子类，我需要将构造函数调用替换为一个普通的函数调用，因为 JavaScript 的构造函数里无法返回子类。于是我使用[[../第 11 章 重构 API/11.8 Replace Constructor with Factory Function|以工厂函数取代构造函数]]（334）。
...
准备好实现多态的类结构后，我就可以继续使用[[第 10 章 简化条件逻辑/10.4 Replace Conditional with Polymorphism|以多态取代条件表达式]]（272）手法了。
...
这是一个简单的例子，但我希望它能让你对“重构怎么做”有一点感觉。例中我已经示范了数种重构手法，包括[[第 6 章 第一组重构/6.1 Extract Function|提炼函数]]（106）、[[第 6 章 第一组重构/6.4 Inline Variable|内联变量]]（123）、[[第 8 章 搬移特性/8.1 Move Function|搬移函数]]（198）和[[第 10 章 简化条件逻辑/10.4 Replace Conditional with Polymorphism|以多态取代条件表达式]]（272）等。
